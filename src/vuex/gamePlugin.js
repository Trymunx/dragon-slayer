...
/*export default function createPlugin(eventemitter, listenedEvents, mutationCallbacks) {
  let mutationMap = new Map();
  addListenedToMap(mutationMap, listenedEvents)
  let all = mutationMap.entries();
  return (store) => {
    for([event, mutation] of all) {
      eventemitter.on(event, (...args) => store.commit(mutation, ...args));
    }
    if(mutationCallbacks) {
      store.subscribe((mutation) => {
        if(Object.keys(mutationCallbacks).includes(mutation.type)) {
          ...
          if(typeof cb === "string") {
            eventemitter.emit(cb, mutation.payload);
          }
