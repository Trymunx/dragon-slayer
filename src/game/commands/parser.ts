...
import { match, MatchErrorType, MatchFailure, MatchSuccess } from "./templates";
...
export type TokeniseError = {
  message: string,
  type: TokeniseErrorType
}

export enum TokeniseErrorType {
  INVALID_TOKEN
}

const invalidTokenError = (token: string) => ({
  message: `I don't understand ${token}`,
  type: TokeniseErrorType.INVALID_TOKEN,
});
...
const contextualise = (tokenStream: TokenStream): Result<ParseError, TokenStream> => {
  ...
  for (let i = 0; i < tokenStream.length; i++) {
    ...
    if (token.type !== TokenType.Word) {
      ...
      if (contextualisedToken !== undefined) {
        ...
        return err(invalidTokenError(token.value));
      }
    }
  }

  return ok(contextualisedTokenStream);
};

// Parsing ---------------------------------------------------------------------
export const run = (input: string): Result<ParseError, MatchSuccess> => {
  ...
  return isOk(contextualisedTokens)
    ? match(contextualisedTokens.value)
    : contextualisedTokens;
};
